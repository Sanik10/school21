...
import ru.springxo.domain.model.*;
import ru.springxo.domain.service.GameService;
...
import ru.springxo.web.model.LeaderboardEntryDto;
...
public class GameController {
    ...
    public GameDto createGame(@RequestParam OpponentType opponentType) {
        ...
        if (opponentType == OpponentType.HUMAN) {
            ...
            playerXId = SystemUsers.COMPUTER_ID;
    ...
    public GameDto joinGame(@PathVariable UUID id) {
        ...
        Game updated = new Game(
                ...
                currentUserId,          // O — присоединившийся
    ...
    private Game handleComputerMove(Game game, UUID currentUserId, MoveRequest move) {
        ...
        if (winnerSymbol == 1) {
            newStatus = GameStatus.FINISHED;
            winnerUserId = SystemUsers.COMPUTER_ID;
    ...
    public List<GameDto> getFinishedGames() {
        UUID currentUserId = getCurrentUserId();
        List<Game> games = gameService.getFinishedGamesForUser(currentUserId);
        return games.stream()
                .map(mapper::toDto)
                .collect(Collectors.toList());
    }
    ...
    public List<LeaderboardEntryDto> getLeaderboard(@RequestParam("limit") int limit) {
        if (limit <= 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "limit must be positive");
        }
        List<LeaderboardItem> items = gameService.getTopPlayers(limit);
        return items.stream()
